//
//  QSFreeActivitiesTableViewCell.m
//  Eating
//
...
#if 0
    //分店名 view.frame.size.width-87.0f-63.0f
    UILabel *addressLabel = [[UILabel alloc] initWithFrame:CGRectMake(77.0f+50.0f, 13.0f, 60.0f, 14.0f)];
    addressLabel.text = @"[体育西店]";
    addressLabel.font = [UIFont systemFontOfSize:12.0f];
    addressLabel.textAlignment = NSTextAlignmentLeft;
    addressLabel.textColor = kBaseLightGrayColor;
    [view addSubview:addressLabel];
    objc_setAssociatedObject(self, &MarLocalNameKey, addressLabel, OBJC_ASSOCIATION_ASSIGN);
#endif
...
{
    //更新活动店铺图片
    [self updateFreeMarIcon:model.storeIcon];
    
    //更新店铺名
    [self updateFreeMarName:[self formatMarName:model.storeName andNickName:model.marOtherName]];
    
    //更新店铺地方名
//    [self updateMarLocalName:model.storeAddress];
    
    //更新距离
    [self updateFreeMarDistance:model.distance];
    
    //更新星星的显示
    [self updateStartLevelImage:[model.starLevel floatValue]];
    
    //更新评分
    [self updateFreemarScore:model.starLevel];
    
    //更新活动日期
    [self updateFreeActivityTime:[self formateActivitiesDate:model.startDate andEndDate:model.endDate]];
    
    //更新状态
    [self updateFreeActivityStatus:model.statusCurrent];
}
...
//更新店铺分店名
- (void)updateMarLocalName:(NSString *)localName
{
    if (localName) {
        UILabel *name = objc_getAssociatedObject(self, &MarLocalNameKey);
        name.text = localName;
    }
}
