...
#define kPortraitHeight 55.f

#define kCalloutWidth   270.0
...
- (void)btnAction
{
    ...
    if (self.onNavButtonHandler) {
        self.onNavButtonHandler(self.item);
    }
}
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if (selected)
    {
        if (self.calloutView == nil)
        {
            ...
            [btn setBackgroundColor:kBaseGreenColor];
            ...
            name.text = [NSString stringWithFormat:@"%@\n%@",self.item.merchant_name,self.item.address];
