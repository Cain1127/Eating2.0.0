...
- (void)initializePageControl {
    ...
    _pageControl.currentPageIndicatorTintColor = HEXCOLOR(0xDA251D);
    _pageControl.pageIndicatorTintColor = HEXCOLOR(0xC0B4A9);
    _pageControl.center = CGPointMake(CGRectGetWidth(_scrollView.frame)*0.8, CGRectGetHeight(_scrollView.frame) - 12.);
...
- (void)loadData {
    ...
    for (NSInteger i = 0; i < _datasourceImages.count; i++) {
        ...
        imageView.image = IMAGENAME(@"首页banner占位图.png");
        imageView.backgroundColor = [UIColor clearColor];
